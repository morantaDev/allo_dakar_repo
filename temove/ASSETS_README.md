# 📦 Guide d'Utilisation des Assets TéMove

## 🎨 Logos

### Fichiers Disponibles

- `assets/logos/temove_logo.svg` - Logo full-color (bleu électrique)
- `assets/logos/temove_logo_monochrome.svg` - Logo monochrome (noir sur blanc)

### Utilisation dans Flutter

```dart
import 'package:temove/widgets/temove_logo.dart';

// Logo full-color avec texte
TeMoveLogo(
  size: 150,
  showText: true,
)

// Logo monochrome
TeMoveLogo(
  size: 150,
  monochrome: true,
)

// Logo compact (pour navigation)
TeMoveLogoCompact(size: 40)

// Logo sans fond (pour fonds colorés)
TeMoveLogoOutline(
  size: 150,
  showText: true,
)
```

## 🎯 Icônes

### Utilisation

```dart
import 'package:temove/assets/icons/app_icons.dart';

// Icône simple
AppIcons.styledIcon(
  icon: AppIcons.ride,
  size: 24,
  isPrimary: true,
)

// Icône avec badge (notifications)
AppIcons.iconWithBadge(
  icon: AppIcons.notification,
  count: 5,
  size: 24,
)

// Icône avec gradient
AppIcons.gradientIcon(
  icon: AppIcons.ride,
  size: 24,
)
```

### Icônes Disponibles

- **Navigation** : `ride`, `booking`, `navigation`, `destination`, `currentLocation`, `search`
- **Fonctionnalités** : `favorite`, `history`, `profile`, `settings`, `notification`, `payment`, `promo`
- **Transport** : `eco`, `comfort`, `comfortPlus`, `carpool`
- **Statut** : `pending`, `accepted`, `inProgress`, `completed`, `cancelled`
- **Communication** : `call`, `message`, `chat`

## 🌐 Favicons

### Génération des Favicons PNG

1. **Installer ImageMagick** : https://imagemagick.org/
2. **Exécuter le script** :
   ```powershell
   cd temove
   .\scripts\generate_favicons.ps1
   ```

### Fichiers Générés

- `web/favicon.svg` - Favicon vectoriel (recommandé)
- `web/favicon.png` - Favicon PNG 32x32
- `web/favicon-16.png` - Favicon 16x16
- `web/favicon-32.png` - Favicon 32x32
- `web/favicon-48.png` - Favicon 48x48
- `web/favicon-192.png` - Favicon 192x192 (Apple Touch Icon)
- `web/favicon-512.png` - Favicon 512x512 (Apple Touch Icon)

## 📱 Icônes d'Application

### Android

Les icônes Android sont générées automatiquement via `flutter_launcher_icons` :

```yaml
flutter_launcher_icons:
  android: true
  ios: false
  image_path: "assets/icons/app_logo.png"
  adaptive_icon_background: "#3B82F6"
  adaptive_icon_foreground: "assets/icons/app_logo.png"
```

### iOS

Les icônes iOS doivent être ajoutées manuellement dans Xcode.

## 🎨 Palette de Couleurs

Voir `IDENTITE_VISUELLE_TEMOVE.md` pour la palette complète.

### Couleurs Principales

- **Bleu électrique** : `#3B82F6` (primaryColor)
- **Violet vibrant** : `#8B5CF6` (secondaryColor)
- **Rose vibrant** : `#EC4899` (accentColor)
- **Turquoise néon** : `#06B6D4` (successColor)
- **Rouge corail** : `#F43F5E` (errorColor)

## 📝 Notes

- Tous les logos sont en SVG pour une scalabilité parfaite
- Les icônes utilisent Material Icons pour la cohérence
- Les favicons SVG sont préférés aux PNG pour le web moderne
- Respecter les guidelines d'espacement (20% de la hauteur du logo)

## 🔗 Liens Utiles

- [Documentation Identité Visuelle](../IDENTITE_VISUELLE_TEMOVE.md)
- [Flutter SVG Package](https://pub.dev/packages/flutter_svg)
- [ImageMagick](https://imagemagick.org/)

